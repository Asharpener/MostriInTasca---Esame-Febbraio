...
// Components
import { StarFlat } from "../sezioni/StarFlat";
...
export default function StarList() {
            ...
            if (location != null) {
                (async () => {
                    ...
                    let row = await NearListRepo.loadStarList().catch((error) => {
                        ...
                        NewAlert.createAlert("Errore", "Impossibile caricare le stelle.", [{ text: "OK", onPress: () => { navigation.navigate('Map') } }]);
                    });
                    console.log("provagiugno" + row.length);
                    setNL(row);
                    setLoading(false);
                })();
            }
    ...
    return (
        <View style={{ flex: 1, flexDirection: 'columns',marginTop: 40 }}>
            {isLoading ?
